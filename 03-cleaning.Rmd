...
We also notice that some species only appear several times. We choose to drop species appearing less than 300 records which represent that they are popular in the research.
```{r}
cnt <- species %>%
        group_by(species) %>%
...
pie1 <- group %>%
        group_by(class) %>%
        count(name = 'count', sort = TRUE)
ggplot(pie1, aes(x = "", y = count, fill = class)) +
