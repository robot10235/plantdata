...
colnames(species)
```
We also notice that some species only appear several times. We need to drop species appearing less than 50 times.
```{r}
cnt <- species %>%
        group_by(species) %>%
        count(name = 'count', sort = TRUE)
tail(cnt, 10)
cnt <- subset(cnt, count >= 50)
new_species <- subset(species, species %in% cnt$species)
```

Finally, save cleaning data.
```{r}
