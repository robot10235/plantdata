...
colnames(species)
```
We also notice that some species only appear several times. We choose to drop species appearing less than 200 records.
```{r}
cnt <- species %>%
        group_by(species) %>%
        count(name = 'count', sort = TRUE)
tail(cnt, 10)
cnt <- subset(cnt, count >= 200)
new_species <- subset(species, species %in% cnt$species)
```

Finally, we get 100 species to investigate.
```{r}
cnt$species
