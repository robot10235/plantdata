...
COPRE(coordinate precision), DP, DPA, EA and ELE are more likely to be missing. It is sad that we cannot use depth and elevation to present one of changes of plants. COUN(coordinate uncertainty in meter) has no missing because we first downloaded data with limitation of COUN smaller than 10000. And SP(species), SK(species key) has no missing data because at chapter 3 we purge all missing species key data. ST(state), Y has no missing at all. ST(state) and Y(year) is much easier to record. 
